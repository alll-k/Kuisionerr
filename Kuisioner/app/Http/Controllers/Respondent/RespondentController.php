...
class RespondentController extends Controller
{
    ...
    {
        // HANYA tampilkan kuesioner yang statusnya 'published'
        $questionnaires = Questionnaire::where('status', 'published')
                                    ->orderBy('created_at', 'desc')
                                    ->get();

        return view('respondent.dashboard', compact('questionnaires'));
    }
    ...
    {
        // Check if user's role matches the questionnaire's target role
        if ($questionnaire->target_role_id !== auth()->user()->role_id) {
