    ...
    <div class="col-md-12">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="text-muted mb-0">Edit pertanyaan untuk kuesioner <strong>{{ $question->questionnaire->title }}</strong>.</p>
            </div>
            <a href="{{ route('admin.questionnaires.show', $question->questionnaire) }}" class="btn btn-secondary">
    ...
    <div class="col-lg-8">
        <div class="card shadow-sm">
            <div class="card-header bg-white border-bottom">
                <h5 class="mb-0">Form Edit Pertanyaan</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.questions.update', $question) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="question_text" name="question_text" rows="3" class="form-control @error('question_text') is-invalid @enderror" required>{{ old('question_text', $question->question_text) }}</textarea>
                        @error('question_text')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select id="sdg_goal_id" name="sdg_goal_id" class="form-select @error('sdg_goal_id') is-invalid @enderror" required>
                                ...
                                @foreach($sdgGoals as $goal)
                                    <option value="{{ $goal->id }}" {{ old('sdg_goal_id', $question->sdg_goal_id) == $goal->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="question_type" name="type" class="form-select @error('type') is-invalid @enderror" onchange="toggleOptions()" required>
                                <option value="text" {{ old('type', $question->question_type) == 'text' ? 'selected' : '' }}>Teks Bebas</option>
                                <option value="multiple_choice" {{ old('type', $question->question_type) == 'multiple_choice' ? 'selected' : '' }}>Pilihan Ganda</option>
                                <option value="scale" {{ old('type', $question->question_type) == 'scale' ? 'selected' : '' }}>Skala</option>
                                <option value="yes_no" {{ old('type', $question->question_type) == 'yes_no' ? 'selected' : '' }}>Ya / Tidak</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    @php
                        $showOptions = old('type', $question->question_type) === 'multiple_choice';
                        $oldOptions = old('options', $question->questionOptions->pluck('option_text')->toArray());
                    @endphp
                    ...
                    <div class="mt-4 d-flex gap-2">
                        ...
                        <a href="{{ route('admin.questionnaires.show', $question->questionnaire) }}" class="btn btn-outline-secondary">
                            Batal
                        </a>
                    </div>
                </form>
